...
import { useDeleteCarMutation, useGetCarsByNameQuery } from "../../store/CarSlice";
...
export default function Cars() {
    ...
    const { data: cars } = useGetCarsByNameQuery({name: `${name}`});
    const [vin, setVin] = useState('');
    const [totalItems, setTotalItems] = useState(0);
    const {data: searchResult} = useGetCarsByNameQuery({name: `${debounce}`}, {
